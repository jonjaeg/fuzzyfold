...
impl LoopDecomposition for MultiPairTable {
    fn for_each_loop<F: FnMut(&NearestNeighborLoop)>(&self, mut f: F) {
                ...
                match mpt[p] {
                    ...
                    MultiStruct::StrandBreak => {
                        if let Some((p5, p3)) = ends {
                            ...
                            let inner;
                            (branches, inner) =
                                branches.iter().copied().partition(|&(_, k)| k <= p5);
        ...
        recurse(
            self,
            Some((0 as NAIDX, (self.len() - 1) as NAIDX)),
            None,
            &mut f,
        );
    }
...
mod tests {
    ...
    fn test_loop_ranges() {
        ...
        assert_eq!(
            vec![2usize, 5, 10, 11, 12, 13, 14, 15],
            mloop.inclusive_unpaired_indices()
        );
        ...
        assert_eq!(
            vec![0usize, 1, 6, 12, 13, 14, 15],
            eloop.inclusive_unpaired_indices()
        );
        ...
        assert_eq!(
            vec![8usize, 9, 10, 11, 12, 13, 20, 1, 2, 6],
            jloop.inclusive_unpaired_indices()
        );
    }
